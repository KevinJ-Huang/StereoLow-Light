...
class SIEN_Model3(BaseModel):
    def __init__(self, opt):
        super(SIEN_Model3, self).__init__(opt)
    ...
    def optimize_parameters(self, step):
        ...
        SR_left, SR_right, SR_left_en, SR_right_en, (M_right_to_left, M_left_to_right), (V_left, V_right) = \
            self.netG(LR_left, LR_right, is_training=1)
        ...
        l_total = loss_SR + 0.4*loss_level4+0.4*loss_pre
        ...
        # self.log_dict['l_pre'] = loss_pre.item()
        self.log_dict['l_lv4'] = loss_level4.item()


    def level_loss(self,LR_left,HR_left,LR_right,HR_right,SR_left,SR_right,SR_left_en,SR_right_en,M_right_to_left,M_left_to_right,V_left,V_right,level):
        b, c, h, w = LR_left.shape
        h, w = h // level, w // level
        Res_left = F.interpolate(torch.abs(HR_left - SR_left_en),scale_factor=1.0/level,align_corners=False,mode='bicubic')
        Res_right = F.interpolate(torch.abs(HR_right - SR_right_en),scale_factor=1.0/level,align_corners=False,mode='bicubic')
