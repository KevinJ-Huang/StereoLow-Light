...
class SIEN_Model2(BaseModel):
    def __init__(self, opt):
        super(SIEN_Model2, self).__init__(opt)
    ...
    def optimize_parameters(self, step):
        ...
        SR_left, SR_right, res2_left, res2_right, res3_left, res3_right,\
        ...
        loss_level4 =  self.level_loss(LR_left,HR_left,LR_right,HR_right,SR_left,SR_right,
                                       M_right_to_left,M_left_to_right,V_left,V_right,4)
        loss_band2 = self.cri_pix(res2_left,F.interpolate(HR_left-LR_left,scale_factor=0.5,align_corners=False,mode='bilinear'))+\
                     self.cri_pix(res2_right,F.interpolate(HR_right-LR_right,scale_factor=0.5,align_corners=False,mode='bilinear'))
        loss_band3 = self.cri_pix(res3_left, F.interpolate(HR_left - LR_left, scale_factor=0.25, align_corners=False,mode='bilinear')) + \
                     self.cri_pix(res3_right, F.interpolate(HR_right - LR_right, scale_factor=0.25, align_corners=False,mode='bilinear'))
        ...
        l_total = loss_SR + 0.4*loss_level4 + loss_band2*0.1 + loss_band3*0.15
    ...
    def level_loss(self,LR_left,HR_left,LR_right,HR_right,SR_left,SR_right,M_right_to_left,M_left_to_right,V_left,V_right,level):
        b, c, h, w = LR_left.shape
        h, w = h // level, w // level
        Res_left = F.interpolate(torch.abs(HR_left - LR_left),scale_factor=1.0/level,align_corners=False,mode='bicubic')
        Res_right = F.interpolate(torch.abs(HR_right - LR_right),scale_factor=1.0/level,align_corners=False,mode='bicubic')
        ...
        SR_left_res = F.interpolate(torch.abs(HR_left - SR_left), scale_factor=1.0/level, align_corners=False,mode='bicubic')
        SR_right_res = F.interpolate(torch.abs(HR_right - SR_right), scale_factor=1.0/level, align_corners=False, mode='bicubic')
