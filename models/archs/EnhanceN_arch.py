...
from models.archs.arch_util import DWT,IWT,space_to_depth,GaussianBlur
...
class AFF(nn.Module):
    def __init__(self, channels, r=4):
        ...
        self.conv = nn.Conv2d(2*channels, channels, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x1, x2):
        xf1 = self.conv(torch.cat([x1, x2],1))
        x_att = self.global_att(xf1)
        xf2 = xf1*x_att
        xo = x1+xf2
...
class Net(nn.Module):
    def __init__(self, upscale_factor=1.0):
        ...
        self.encoder0 = nn.Conv2d(3, G0, 3, padding=1, stride=1)
        self.encoder1 = RDB(G0=G0, C=4, G=G)
        ...
        self.fusion3 = nn.Sequential(
            RDB(G0=8*G0, C=4, G=2*G),
            CALayer(8*G0),
            nn.Conv2d(8*G0, 4*G0, kernel_size=1, stride=1, padding=0, bias=True))


        self.fusion2 = nn.Sequential(
            RDB(G0=4*G0, C=4, G=G),
            CALayer(4*G0),
            nn.Conv2d(4*G0, 2*G0, kernel_size=1, stride=1, padding=0, bias=True))
        ...
        self.decoder1 =  RDB(G0=G0, C=4, G=G)
        self.AFF1 = AFF(G0)
    ...
    def forward(self, x_left, x_right, is_training):
        ...
        left_ef1 = self.encoder1(self.encoder0(x_left))
        right_ef1 = self.encoder1(self.encoder0(x_right))
        ...
        buffer_leftF3 = self.fusion3(torch.cat([left_ef3, buffer_leftT3], dim=1))
        buffer_rightF3 = self.fusion3(torch.cat([right_ef3, buffer_rightT3], dim=1))
        ...
        buffer_leftF2 = self.fusion2(torch.cat([left_ef2, buffer_leftT2], dim=1))
        buffer_rightF2 = self.fusion2(torch.cat([right_ef2, buffer_rightT2], dim=1))

        left_df2, _ = self.decoder2(self.up2(buffer_leftF3))
        left_df2 = self.AFF2(left_df2, buffer_leftF2)
        right_df2, _ = self.decoder2(self.up2(buffer_rightF3))
        right_df2 = self.AFF2(right_df2, buffer_rightF2)
        ...
        buffer_leftF1 = self.fusion1(torch.cat([left_ef1, self.IWT(buffer_leftT1)], dim=1))
        buffer_rightF1 = self.fusion1(torch.cat([right_ef1, self.IWT(buffer_rightT1)], dim=1))

        left_df1 = self.decoder1(self.up1(left_df2))
        left_df1 = self.AFF1(left_df1, buffer_leftF1)
        right_df1 = self.decoder1(self.up1(right_df2))
        right_df1 = self.AFF1(right_df1, buffer_rightF1)
        ...
        out_left = self.decoder0(left_df1) + self.Img_up(res2_left)
        out_right = self.decoder0(right_df1) + self.Img_up(res2_right)
        ...
        if is_training == 1:
            ...
            return out_left, out_right, res2_left, res2_right, res3_left, res3_right,\
