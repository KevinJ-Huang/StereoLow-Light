    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt['logger']['print_freq'] == 0:
                ...
                total_sr += logs['l_sr']
                total_pre += logs['l_pre']
                total_band2 += logs['l_band2']
                total_band3 += logs['l_band3']
                total_lv4 += logs['l_lv4']
                total_psnr += logs['psnr']
                mean_total = total_loss / print_iter
                mean_sr = total_sr / print_iter
                mean_band2 = total_band2 / print_iter
                mean_band3 = total_band3 / print_iter
                mean_lv4 = total_lv4 / print_iter
                mean_psnr = total_psnr / print_iter
                mean_pre = total_pre / print_iter
                message += '{:s}: {:.4e} '.format('mean_total_loss', mean_total)
                message += '{:s}: {:.4e} '.format('mean_sr_loss', mean_sr)
                message += '{:s}: {:.4e} '.format('mean_band2_loss', mean_band2)
                message += '{:s}: {:.4e} '.format('mean_band3_loss', mean_band3)
                message += '{:s}: {:.4e} '.format('mean_lv4_loss', mean_lv4)
                message += '{:s}: {:.4e} '.format('mean_pre_loss', mean_pre)
